...
The `contact-content.json` file contains all the hardcoded content from the website's Contact page, structured to match the Sanity `contactPage` schema.
...
```bash
# Make sure you have Sanity credentials in .env.local
npx sanity dataset import sanity-data/contact-content.json
```

**Option 2: Via the Sanity Studio UI**

1. Navigate to `http://localhost:3000/studio`
2. In the left sidebar, click on **Contact Page**
3. Manually create/update the document with fields from `contact-content.json`
...
- All field values match the currently hardcoded defaults in `app/contact/page.tsx`
- The document ID is `contact-page` and type is `contactPage`
- After importing, the website will fetch this data from Sanity instead of using hardcoded fallbacks
